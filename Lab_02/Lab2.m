...
a = 1/N *  P*inv(D)*transpose(P)*transpose(X_train)*y_train;

stima = X_train * a;
stima2 = X_test * a;

figure
hist(y_test - stima2,50)

figure
hist(y_train - stima, 50)
...
a_hat_L = 1/N * P_L * inv(D_L) * transpose(P_L) * transpose(X_train) *y_train;
...
% [N,X] = hist(stima2 - y_test, 10);
% figure
% hist(stima2 - y_test, 10)
% grid on